...
public record GetStudentByStudentIdQuery(string StudentId) : IRequest<Result<StudentDto>>;

public class GetStudentByStudentIdQueryHandler(
    IStudentRepository studentRepository)
    : IRequestHandler<GetStudentByStudentIdQuery, Result<StudentDto>>
{
    public async Task<Result<StudentDto>> Handle(GetStudentByStudentIdQuery query, CancellationToken cancellationToken)
    {
        try
        {
            // Validate input
            ...
            // Get student by StudentId with all related details
            var student = await studentRepository.GetByStudentIdWithDetailsAsync(query.StudentId);
            if (student is null)
            {
                return Result<StudentDto>.Failed(
                    Error.NotFound(
        ...
        catch (Exception ex)
        {
            ...
            // _logger.LogError(ex, "Error getting student by StudentId {StudentId}", query.StudentId);
